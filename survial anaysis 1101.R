...
#censoring_rate from different censoring_time and sample size
parameter_summary <- metadata_summary %>%
  group_by(n_points, censoring_time) %>%
  summarise(
    mean_censoring_rate = mean(censoring_rate),
    mean_time_obs = mean(mean_time_obs),
    .groups = "drop"
  )
...
ggplot(parameter_summary, aes(x = censoring_time, 
  ...
  labs(
    ...
    y = "Mean Censoring Rate",
    color = "Sample Size"
  ) +
  theme_minimal(base_size = 14)

#censoring_rate in all covariance combination
...
#ggplot for the all covariance combination
