...
### 5. Visualization of Model Performance (Automated Table + Visualization)

#unify model names across summaries
overall_raw$model <- recode(overall_raw$model,
                            "aft_raw" = "AFT (Weibull)",
                            "cox_raw" = "Cox",
                            "naive_raw" = "Naive")
...
#combine all into one summary table automatically
measure_summary <- overall_raw %>%
  rename(MSE = Mean_MSE, Bias = Mean_MedDiff) %>%
  left_join(overall_log %>% rename(MSE_log = Mean_MSE_log,
                                   Bias_log = Mean_MedDiff_log),
            by = "model") %>%
  left_join(overall_coverage, by = "model")

#print the table for report
...
measure_long <- measure_summary %>%
  ...
  mutate(
    Metric_Group = case_when(
      grepl("MSE", Metric) ~ "Mean Squared Error",
      grepl("Bias", Metric) ~ "Median Survival Time Bias",
      grepl("Coverage", Metric) ~ "95% Confidence Interval Coverage"
    )
  )
...
final_plot <- ggplot(measure_long, aes(x = model, y = Value, fill = Metric)) +
  ...
  geom_hline(
    data = subset(measure_long, Metric_Group == "95% Confidence Interval Coverage"),
    aes(yintercept = 0.95),
    color = "red", linetype = "dashed", linewidth = 1
  ) +
  geom_hline(
    data = subset(measure_long, Metric_Group == "Median Survival Time Bias"),
    aes(yintercept = 0),
