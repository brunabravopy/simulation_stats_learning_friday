...
## CI Coverage (95%) calculation
## =========================================================
calc_coverage <- function(pred, true) {
  ci_low <- quantile(pred, 0.025, na.rm = TRUE)
  ci_high <- quantile(pred, 0.975, na.rm = TRUE)
  as.numeric(true >= ci_low & true <= ci_high)
}

coverage_summary <- res_long_raw %>%
  group_by(dataset_name, model) %>%
  summarise(
    coverage_95 = calc_coverage(pred, true_time),
    .groups = "drop"
  )
...
### 5. Visualization of Performance Measures
library(ggplot2)

# Combine all measures into a single summary table
measure_summary <- overall_raw %>%
  rename(MSE = Mean_MSE, Bias = Mean_MedDiff) %>%
  left_join(overall_log %>% rename(MSE_log = Mean_MSE_log, Bias_log = Mean_MedDiff_log),
            by = "model") %>%
...
# Define order for visual consistency
measure_long$model <- factor(measure_long$model,
                             levels = c("AFT (Weibull)", "Cox", "Naive"))

# Visualization 1: MSE (raw vs log)
p_mse <- measure_long %>%
  filter(metric %in% c("MSE", "MSE_log")) %>%
  ggplot(aes(x = model, y = value, fill = metric)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.6) +
  labs(title = "Mean Squared Error (Raw vs Log Scale)",
       x = "Model", y = "Mean MSE") +
  theme_minimal(base_size = 14) +
...
# Visualization 2: Bias (Median Difference)
p_bias <- measure_long %>%
  filter(metric %in% c("Bias", "Bias_log")) %>%
  ggplot(aes(x = model, y = value, fill = metric)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.6) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
  labs(title = "Median Survival Time Bias (Raw vs Log Scale)",
       x = "Model", y = "Median Difference") +
  theme_minimal(base_size = 14) +
  scale_fill_manual(values = c("#59A14F", "#E15759")) +
  theme(legend.title = element_blank(),
        plot.title = element_text(face = "bold", size = 16))

# Visualization 3: 95% CI Coverage
p_cov <- measure_long %>%
  filter(metric == "Mean_Coverage_95") %>%
  ggplot(aes(x = model, y = value, fill = model)) +
  geom_bar(stat = "identity", width = 0.6) +
  geom_hline(yintercept = 0.95, color = "red", linetype = "dashed", linewidth = 1) +
  labs(title = "95% Confidence Interval Coverage",
       x = "Model", y = "Coverage Probability") +
  theme_minimal(base_size = 14) +
  scale_fill_manual(values = c("#76B7B2", "#FF9DA7", "#BAB0AC")) +
  theme(legend.position = "none",
        plot.title = element_text(face = "bold", size = 16))

# Combine all three plots in one panel
library(patchwork)
final_plot <- (p_mse / p_bias / p_cov) +
  plot_annotation(
