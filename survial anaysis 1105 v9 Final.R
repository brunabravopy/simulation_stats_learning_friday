...
### 5. Visualization of Model Performance (No Log Columns, No Barplot)
...
## Combine only median-based and coverage metrics (no log)
measure_summary <- overall_median %>%
  rename(MSE = Median_MSE, Bias = Median_Bias) %>%
  left_join(overall_coverage, by = "model")

cat("\n==== Combined Model Performance Table (Median-based Evaluation, No Log) ====\n")
print(measure_summary)


### (Removed) → Deleted the big barplot (final_plot) part
